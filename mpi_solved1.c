...
if (rank == 0) {
  ...
  dest = rank + 1;
  printf("rank:%d -- dest:%d",rank,dest);
  source = dest;
  tag = rank;
  rc = MPI_Send(&outmsg, 1, MPI_CHAR, dest, tag, MPI_COMM_WORLD);
  printf("Sent to task %d...\n",dest);
  rc = MPI_Recv(&inmsg, 1, MPI_CHAR, 1, 0, MPI_COMM_WORLD, &Stat);
  printf("Received rank0 from task %d...\n",source);
  }
  ...
  source = dest;
  tag = rank-1;
  printf("receiving -- rank:%d -- dest:%d",rank,dest);
